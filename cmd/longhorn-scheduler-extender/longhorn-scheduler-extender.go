...
import (
	...
	"github.com/blampe/longhorn-scheduler/pkg/consts"
	_ "github.com/blampe/longhorn-scheduler/pkg/driver"
)
...
func main() {
	...
	app.Usage = "Longhorn scheduler extender for Kubernetes"
	app.Action = run
...
func run(c *cli.Context) {
	...
	d, err := volume.Get(volume.CSIDriverName)
	if err != nil {
		log.Fatalf("Error getting Scheduler Driver %v: %v", volume.CSIDriverName, err)
	}

	if err = d.Init(nil); err != nil {
		log.Fatalf("Error initializing Scheduler Driver %v: %v", volume.CSIDriverName, err)
	}
